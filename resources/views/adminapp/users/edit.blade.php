...
<form method="POST" action="{{route('admin.users.update', [ 'id' => $data->id ])}}">
  ...
  <div class="mb-3">
    ...
    <!-- <div id="emailHelp" class="form-text">We'll never share yovalue="{{ $data->name }}"ur email with anyone else.</div> -->
  </div>
  <div class="mb-3">
  ...
  <div class="mb-3">
        <select name="gender" class="form-select" aria-label="Default select example">
            <option value="0" {{ $data->role == 1 ? "selected" :  ""}}>nam</option>
            <option value="1" {{ $data->role == 0 ? "selected" :  ""}}>nữ</option>
        </select>
  </div>
  <div class="mb-3">
